...
`/storia` risponde 200, `npx tsc --noEmit` pulito all'ultimo controllo. Il televisore è visibile solo da 1400px di larghezza viewport in su (media query `.side-spotlight`), fisso a sinistra, verticalmente centrato. Verificato con screenshot+zoom in Chrome, non solo assunto:
- L'allineamento schermo/cornice regge anche col tilt 3D.
- Il bob e il pulsare dell'ombra sono visibili e in fase.
- Lo scanline e la sabbia (SVG feTurbulence) sono visibili.
- Il lampo di accensione scatta esattamente a scrollY 100px (letto dall'overlay `?debug=1`, non stimato).
- A scrollY 400px lo schermo è ancora tutta sabbia; a 500px la prima immagine (foto casa, `Steadycam_ilCentro3-scaled.jpg`) è a piena opacità — confermato con lo stesso overlay di debug.
